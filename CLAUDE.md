...
**竞技场生存（arena_survival）反例（当前代码未收敛，勿效仿）：**
- easy → normal：实际约 11 处差异（场地 24→18、波次 5→7、每波虫子 4→5、虫子加 spitter、精英虫型升级、Boss 升级、弹药 30→20、掩体 18→12、道具间隔 9s→12s、道具少 1 种、波次编排新增 Boss 波），同时违反规则 1 与规则 4
- normal → hard：场地 18→14 + 波次 7→10（2 条，符合规则）
...
> 原列出的 `世界添加说明.md`、`魔法技能增加说明.md` 已从仓库删除（git 历史中亦无），**勿寻找**。新世界/魔法技能添加流程以实际代码为准：世界框架 `maps/amap/world/framework.lua`、副本框架 `maps/amap/instance/instance.lua`。
