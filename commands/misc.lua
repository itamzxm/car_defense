...
local function clear_corpses(cmd)
    ...
    for _, entity in pairs(player.physical_surface.find_entities_filtered {area = radius, type = 'corpse'}) do
...
commands.add_command(
    'coins',
    'Shows coin count for all players.',
    function()
        local player = game.player
        if not (player and player.valid) then
            return
