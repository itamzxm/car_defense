...
local cleanup_legacy_gui = LegacyCleanup.cleanup_legacy_gui
...
local function update_gui(player)
    ...
    -- 更新UI元素
    frame.best_record.caption = {'amap.map_info_bar_best_record', best_record}
    frame.best_record.tooltip = {'amap.best_record', best_record}

    frame.biter_target.caption = {'amap.map_info_bar_biter_target', car_name}
    frame.biter_target.tooltip = {'amap.biter_target'}

    frame.landmine.caption = {'amap.map_info_bar_landmine', format_number(this.now_mine, true), format_number(this.max_mine, true)}
    frame.landmine.tooltip = {'amap.land_mine_placed'}

    frame.flame_turret.caption = {'amap.map_info_bar_flame_turret', format_number(this.flame, true), format_number(this.max_flame, true)}
...
local function handle_main_button_click(event)
    ...
    if flow[CONST.MAIN_FRAME] then
        ...
        if info.visible then
            info.visible = false
            TopBar.set_button_visible(player.index, CONST.MAIN_FRAME, false)
        else
            ...
            info.visible = true
            TopBar.set_button_visible(player.index, CONST.MAIN_FRAME, true)
            update_gui(player)
        end
    else
