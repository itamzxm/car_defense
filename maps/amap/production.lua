...
local function produce(factory, train)
  if factory.active then
    ...
    local progress_excess = factory.progress - List[id].base_time * 60*2
    local pollution_coef = 0.1 + factory.tier*2
    if progress_excess >= 0  then
      ...
      if train then
        ...
        -- 在Factorio 2.0中，products_finished属性已被废弃，使用生产统计代替
...
function Public.produce_assemblers()
  ...
  for key, factory in pairs(production.train_assemblers) do
    if factory.tier > 0 or this.jjc ==2  then
...
local name = List[id].recipe_override or List[id].name
  local science_packs = {
  ...
  if science_packs[name] then
     tag = game.forces.player.add_chart_tag(entity.surface, {
