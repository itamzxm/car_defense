...
--       每 chunk 中心生成 1 块 2×2 矿（40% 方解石 5k / 60% 废料 20k 总量），
--       1/30 虫巢（山谷密度 3 倍），无撼地虫。
--
...
local function on_world_start(world_number)
    ...
    --（历史最高波数由 tank.lua 写入 map.map_record[21]；概率读 world_bonus_type 的 0.1/0.02）
    local map = diff.get()
    local record = map.map_record and map.map_record[21] or 0
    local p = 0
    if record >= 1500 then
        local bt = World.get_field(21, 'world_bonus_type')
        local steps = math.floor((record - 1500) / 500)
        p = math.floor(((bt and bt.base_value or 0) + (bt and bt.growth_value or 0) * steps) * 100)
    end
...
World.register(21, {
    ...
    -- 火星区（y ≤ -480）宝箱物品数量倍率（world_helpers.rand_box → Loot.add mult 消费）
    mars_chest_mult = 3,

    -- 禁用随机野外市场分支（world_helpers.ywjz 消费；市场只生成于岩浆区市场块）
    wild_market_disabled = true,

    -- 火星区宝箱/史诗宝箱出现概率倍率（world_helpers.ywjz 消费）
    mars_box_weight_mult = 2,
