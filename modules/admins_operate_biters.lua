...
local function get_evo(force)
  local evo = math_floor(game.forces["enemy"].get_evolution_factor() * 20)
  local nests = math_random(1 + evo, 2 + evo * 2 )
end

local function place_nest_near_unit_group(group)
  if not storage.biter_command.buildings then return false end
...
local function build_worm(group)
  ...
  for i = 1, 5, 1 do
	...
	local worm = worm_raffle[math_random(1 + math_floor(group.force.get_evolution_factor() * 8), math_floor(1 + group.force.get_evolution_factor() * 16))]
	if not position then return false end
...
local function pan(group, source_player)
	source_player.set_controller({type = "remote", position = group.position})
end
...
local function teleport(group, source_player)
  if source_player.admin or storage.biter_command.teleporting  then
     ...
     flying_text("Teleporting is disabled", nil, source_player.physical_position, source_player)
   end
end
...
local function movetome(group, source_player)
  group.set_command(move_to(source_player.physical_position, defines.distraction.none))
...
local function patroltome(group, source_player)
  group.set_command(move_to(source_player.physical_position, defines.distraction.by_enemy))
  flying_text(nil, 1, group.position, source_player)
end
...
local function attackenemiesaroundme(group, source_player)
  group.set_command(attackareacommand(source_player.physical_position))
  flying_text(nil, 3, group.position, source_player)
end

local function attackobstaclesaroundme(group, source_player)
  local commands = attackobstaclescommand(source_player.physical_surface, source_player.physical_position)
  if #commands > 1 then
  group.set_command({
    type = defines.command.compound,
    structure_type = defines.compound_command.return_last,
...
local function addunitsaroundme(group, source_player)
  local units = source_player.physical_surface.find_entities_filtered{position = source_player.physical_position, radius = 50,type = "unit", force = group.force}
  for i = 1, #units, 1 do
...
local function addunits(group, source_player)
  local units = source_player.physical_surface.find_entities_filtered{position = group.position, radius = 50,type = "unit", force = group.force}
  for i = 1, #units, 1 do
    group.add_member(units[i])
  end
end
...
local function creategroup(source_player)
  source_player.physical_surface.create_unit_group{position = source_player.physical_position, force = source_player.force}
  flying_text("Unit group created", nil, source_player.physical_position, source_player)
end
----------------------direction panel-----------------
...
local function on_unit_group_created(event)
  if event and event.group and event.group.valid then
    storage.biter_command.active_unit_groups[event.group.id] = {id = event.group.id, group = event.group}
    refresh_panel()
  end
end
...
local function on_unit_removed_from_group(event)
  if event and event.group and event.group.valid then
    if #event.group.members == 1 then
      storage.biter_command.active_unit_groups[event.group.id] = nil
      refresh_panel()
    end
  end
end
