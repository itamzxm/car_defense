...
local function on_player_rotated_entity(event)
	if not storage.level_unlocks then return end	
	...
	if not storage.level_unlocks[position_string] then return end
if storage.level_unlocks[position_string].unlocked then return end
storage.level_unlocks[position_string].unlocked = true
		
	if not storage.area_tiles_unlock_schedule then storage.area_tiles_unlock_schedule = {} end	
...
local function on_tick(event)
	if not storage.area_tiles_unlock_schedule then return end
	...
	create_particles(storage.area_tiles_unlock_schedule[#storage.area_tiles_unlock_schedule].surface, storage.area_tiles_unlock_schedule[#storage.area_tiles_unlock_schedule].tile.position)
	
	storage.area_tiles_unlock_schedule[#storage.area_tiles_unlock_schedule] = nil		
	if #storage.area_tiles_unlock_schedule == 0 then storage.area_tiles_unlock_schedule = nil end
end
