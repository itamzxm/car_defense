...
function Public.reset_difficulty_poll(tbl)
    if tbl then
        ...
        this.difficulties[1].count = #game.connected_players
        set_difficulty()
...
local function on_player_joined_game(event)
    ...
    if game.tick < this.difficulty_poll_closing_timeout then
        if not this.difficulty_player_votes[player.name] then
            this.difficulties[1].count = this.difficulties[1].count + 1
            this.difficulty_player_votes[player.name] = {voted = true, index = 1}
            set_difficulty()
            Public.difficulty_gui()
            game.print(player.name .. ' has voted for ' .. this.difficulties[1].name .. ' difficulty!', this.difficulties[1].print_color)
        end
    else
        if player.gui.center['difficulty_poll'] then
            player.gui.center['difficulty_poll'].destroy()
