...
function(data)
  ...
  for i=1,10 do
    local  e =  biter.surface.create_entity(
    {
      name ='laser',
      position = source,
      force = 'enemy',
  ...
  if biter and biter.valid then
    ...
    local amount = rpg_extra.rpg_xp_yield[name]
...
function Public.lightning_chain(position, surface,player)
  ...
  local temp_pos
  if biters[1] and biters[1].valid  then
    temp_pos= biters[1].position
    for i=1,10 do
  ...
  biters = surface.find_entities_filtered{position = temp_pos, radius = 20, type='unit' , force = game.forces.enemy,limit=50}
  ...
  for k,biter in pairs(biters) do
    local dist = (position.x-biter.position.x)^2+(position.y-biter.position.y)^2
    paixu[k]={}
    paixu[k].biter=biter
    paixu[k].dist=dist
  end

  table.sort(paixu, function(a, b)
...
--table.sort(paixu,)
  ...
  for i = 1, 9 do
    ...
    if biter and biter.valid then
      ...
      Task.set_timeout_in_ticks(i*15, lightning, data)
    end
    temp_pos=biter.position
  end
  return true
end
