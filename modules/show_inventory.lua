    ...
    for name, opts in pairs(panel_type) do
        ...
        if caption == 'Armor' then
            if target.get_inventory(5)[1].grid then
                ...
                for _, item_data in pairs(p_armor) do
                    local k = item_data.name
                    local v = item_data.count
                    local armor_gui =
                        flow.add(
                        {
                            type = 'sprite-button',
...
local function open_inventory(source, target)
    ...
    local function convert_contents(contents)
        local result = {}
        for _, item_data in pairs(contents) do
            result[item_data.name] = item_data.count
        end
        return result
    end
    ...
    local ammo = convert_contents(target.get_inventory(defines.inventory.character_ammo).get_contents())
    local trash = convert_contents(target.get_inventory(defines.inventory.character_trash).get_contents())
