...
local function can_units_spawn()
    -- World 框架：wave_attack_settings（世界16「平凡之日」等按波数集中进攻的世界）
    --   every：仅当波数为 every 的整数倍时才刷虫进攻，其余波只囤积威胁不刷虫
    --   require_fortress：为 true 时，无存活堡垒（baolei_count<=0）不进攻
    -- 注意：threat 在 set_next_wave 中照常累积，不受此拦截影响
    local this = WPT.get()
    local wave_attack = World.get_field(this and this.world_number, 'wave_attack_settings')
    if wave_attack then
        local every = wave_attack.every or 50
        local wave_number = WD.get('wave_number') or 0
        if wave_number <= 0 or wave_number % every ~= 0 then
            return false
        end
        if wave_attack.require_fortress and (this.baolei_count or 0) <= 0 then
            return false
        end
    end
