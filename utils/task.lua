...
local function on_tick()
    ...
    for i = 1, get_task_per_tick(tick) do
        ...
        if task ~= nil then
            ...
            local success, result = xpcall(Token_get(task.func_token), trace, task.params)
            if not success then
                if _DEBUG then
                    error(result)
                else
    ...
    while callback ~= nil and tick >= callback.time do
        local success, result = xpcall(Token_get(callback.func_token), trace, callback.params)
        if not success then
            if _DEBUG then
                error(result)
            else
