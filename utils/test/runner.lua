...
--- Prints to the player if one exists, otherwise to the server log.
-- Allows running the test runner without any connected player via RCON.
...
local function print_summary(data)
    ...
    say(data.player, table.concat {pass_count, ' of ', data.count, ' tests passed.'})
end
...
local function print_error(player, test_name, error_message)
    say(player, table.concat {"Failed - '", test_name, "': ", tostring(error_message)}, {color = {r = 1}})
end

local function print_success(player, test_name)
    say(player, table.concat {"Passed - '", test_name, "'"}, {color = {g = 1}})
end

local function print_hook_error(hook)
    say(hook.context.player, table.concat {'Failed ', hook.name, " hook -':", tostring(hook.error)}, {color = {r = 1}})
end

local function print_teardown_error(context, name, error_message)
    say(context.player, table.concat {'Failed ', name, " teardown -':", error_message}, {color = {r = 1}})
end
...
local function do_termination(data)
    ...
    say(data.player, 'Test run canceled due to stop on first error policy.')
