...
local down_arrow = '▼'
local right_arrow = '►'
-- Colors aligned with maps/amap/gui_styles.lua (COLORS.ORANGE / COLORS.WHITE)
local color_success = {g = 1}
local color_failure = {r = 1}
local color_selected = {r = 1, g = 0.647, b = 0}
local color_default = {r = 0.88, g = 0.88, b = 0.88}
...
local function find_module_by_id(module, id)
    if module.id == id then
        return module
    end

    for _, child in pairs(module.children) do
        local found = find_module_by_id(child, id)
        if found then
            return found
        end
    end
...
local function find_test_by_id(module, id)
    for _, test in pairs(module.tests or {}) do
        if test.id == id then
            return test
        end
    end
    ...
    for _, child in pairs(module.children) do
        local found = find_test_by_id(child, id)
        if found then
            return found
        end
    end
...
local function get_selected_test_info(player_index)
    local test_info = selected_test_info_by_player_index[player_index]
    if test_info == nil then
        return nil
    end

    local root = Builder.get_root_modules()

    if test_info.type == info_type_test then
        return {type = info_type_test, test = find_test_by_id(root, test_info.id)}
    elseif test_info.type == info_type_module then
        return {type = info_type_module, module = find_module_by_id(root, test_info.id)}
    end
...
local function get_text_box_error(player_index)
    local test_info = get_selected_test_info(player_index)
    if test_info == nil then
        return ''
    end
...
local function is_test_selected(test, player_index)
    ...
    local info_test = test_info.test
...
local function is_module_selected(module, player_index)
    ...
    local info_module = test_info.module
...
GuiDispatcher.register_click(
    ...
    function(event)
        ...
        if is_selected then
            selected_test_info_by_player_index[player_index] = {type = info_type_module, id = module.id}
        end
...
GuiDispatcher.register_click(
    ...
    function(event)
        ...
        if is_selected then
            selected_test_info_by_player_index[player_index] = {type = info_type_test, id = test.id}
        end
...
GuiDispatcher.register_click(
    ...
    function(event)
    local test_info = get_selected_test_info(event.player_index)
    if test_info == nil then
        return
    end
...
GuiDispatcher.register_checked_state_changed(
